...
| Change the emergency provider model | the single `tier: openrouter` registry entry |
...
| `hermes-openrouter-fallback` | OpenRouter emergency egress | one provider key |
...
The one `tier: openrouter` registry entry is a fixed LiteLLM fallback for
`hermes-default`, not a caller-selectable catalog. LiteLLM uses it only after
the scoped local retry policy; the provider credential is injected into the
LiteLLM service environment and is never available to Hermes.
