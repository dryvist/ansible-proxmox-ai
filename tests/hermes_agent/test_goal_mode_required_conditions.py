"""Every expected snippet is present in the combined required-patch assert.

Split from test_goal_mode_source_postconditions.py (over the repo's per-file
token budget, a real seam): this file pins WHICH conditions the task's
`ansible.builtin.assert` must contain; the postconditions file proves the
`_source_postconditions` guard function actually goes red for a dropped one.
"""

from __future__ import annotations

from conftest import _combined_assert_task, _task
