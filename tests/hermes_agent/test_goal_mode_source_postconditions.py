"""The fail-closed postcondition guard actually catches each specific mutation.

Split from test_goal_mode_required_conditions.py (over the repo's per-file
token budget): that file pins which snippets the combined assert TASK
expects; this one proves the `_source_postconditions` guard function itself
goes red for every mutation it exists to catch, one at a time.
"""
...
def test_source_postconditions_reject_each_dropped_patch() -> None:
